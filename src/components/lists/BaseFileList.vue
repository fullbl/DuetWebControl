...
<template>
  <div>
    ...
    <v-data-table
      ...
      :class="{
        'empty-table-fix': !innerFilelist.length,
        'loading-cursor': isLoading,
      }"
    >
      <template #progress>
        <slot name="progress">
            ...
            <template v-else-if="header.unit === 'filaments'">
              <v-tooltip
                ...
                :disabled="
                  ...
                  props.item[header.value].length <= 1
                "
              >
    ...
    >
      <v-list>
        ...
        <v-list-item
          v-show="!noPreview && innerValue.length === 1 && filesSelected"
...
export default {
  ...
  computed: {
    ...
    isMounted() {
      ...
      return (
        volume >= 0 &&
        volume < this.volumes.length &&
        this.volumes[volume].mounted
      );
    },
    defaultHeaders() {
      return [
    ...
    isLoading() {
      return (
        this.loading ||
        this.innerLoading ||
        this.doingFileOperation ||
        this.innerDoingFileOperation
      );
    },
    foldersSelected() {
    ...
    canEditFile() {
      return (
        this.innerValue.length > 0 && this.innerValue[0].size < maxEditFileSize
      );
    },
    canPreviewFile() {
      if (this.innerValue.length === 0) return false;
      return this.innerValue[0].name.includes(".svg");
    },
    noItemsText() {
  ...
  methods: {
    ...
    toggleAll() {
      this.innerValue = this.innerValue.length
        ? []
        : this.innerFilelist.slice();
    },
    ...
    async loadDirectory(directory) {
      ...
      if (
        ...
        this.innerDirectory =
          volume === Path.getVolume(this.initialDirectory)
    ...
    displayLoadingValue(item, prop, precision, unit = "") {
      ...
      if (!item[prop]) {
        return this.$t(
          item[prop] === undefined ? "generic.loading" : "generic.noValue"
        );
      }
    ...
    onItemDragStart(item, e) {
      ...
      Array.from(tableClone.tBodies[0].rows).forEach(function (row) {
        ...
        if (
          itemsToDrag.some(
            (item) => (item.isDirectory ? "*" : "") + item.name === filename
          )
        ) {
      ...
      tableClone.style.backgroundColor = this.$vuetify.theme.dark
        ? "#424242"
        : "#FFFFFF";
      tableClone.style.opacity = 0.7;
      tableClone.style.position = "absolute";
      tableClone.style.pointerEvents = "none";
      Array.from(tableClone.querySelectorAll('[class^="v-ripple"]')).forEach(
        function (item) {
          item.classList = Array.from(item.classList).filter(
            (c) => !c.startsWith("v-ripple")
          );
        }
      );
      ...
      const y =
        e.clientY - e.target.closest("tr").getClientRects()[0].top + offsetY;
      ...
      if (!this.noDragDrop && item.isDirectory) {
        ...
        if (jsonData) {
          ...
          if (
            data.type === "dwcFiles" &&
            !data.items.some(
              (dataItem) => dataItem.isDirectory && dataItem.name === item.name
            )
          ) {
            e.preventDefault();
            e.stopPropagation();
        ...
        if (
          ...
          for (let i = 0; i < data.items.length; i++) {
            ...
            try {
              ...
              this.$makeNotification(
                "error",
                `Failed to move ${data.items[i].name} to ${directory}`,
                e.message
              );
              break;
            }
    ...
    async download(item) {
      try {
        const filename =
          item && item.name ? item.name : this.innerValue[0].name;
        const blob = await this.machineDownload({
    ...
    async edit(item) {
      try {
        ...
        const response = await this.machineDownload({
          filename,
          type: "text",
          showSuccess: false,
        });
      ...
      for (let i = 0; i < items.length; i++) {
        try {
          ...
          this.innerFilelist = this.innerFilelist.filter(
            (file) =>
              file.isDirectory !== item.isDirectory || file.name !== item.name
          );
          ...
          this.$makeNotification(
            "error",
            this.$t("notification.delete.errorTitle", [items[i].name]),
            items[i].isDirectory
              ? this.$t("notification.delete.errorMessageDirectory")
              : e.message
          );
        }
      }
      ...
      if (deletedItems.length) {
        this.$log(
          ...
          deletedItems.length > 1
            ? this.$t("notification.delete.successMultiple", [
                deletedItems.length,
              ])
    ...
    async downloadZIP(items) {
      ...
      let downloadedFiles;
      try {
        downloadedFiles = await this.machineDownload({
          files,
    ...
    filesOrDirectoriesChanged({ machine, files }) {
      if (
        ...
        Path.filesAffectDirectory(files, this.directory)
      ) {
  ...
  mounted() {
    ...
    this.$root.$on(
      Events.filesOrDirectoriesChanged,
      this.filesOrDirectoriesChanged
    );
  },
  beforeDestroy() {
